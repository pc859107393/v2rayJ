...
public class LogInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (null != requestBody) {
            requestMediaType = requestBody.contentType();
        }
        //记录请求时间
        ...
        return response.newBuilder()
                .body(okhttp3.ResponseBody.create(null != resultBody ? resultBody : new byte[0], responseMediaType))
                .build();
    }
    ...
    private String requestBodyLog(RequestBody requestBody, MediaType mediaType) throws IOException {
        ...
        if (null != requestBody) {
            //如果是文件请求，固定日志打印
            if (null != mediaType && "form-data".equals(mediaType.subtype())) {
                return "files";
            }
            if (null != mediaType && "octet-stream".equals(mediaType.subtype())) {
                return "file";
            }
            //如果是字符串类型的请求
            if (null != mediaType && pattern.matcher(mediaType.toString().toLowerCase()).find()) {
                ...
                if (charset != null) {
                    ...
                    if (body.length() > 1024 * 2) {
                        body = body.substring(0, 1024 * 2);
                    }
                }
                // 如果是表单请求，则decode一下，规避中文乱码
                if (!StringUtils.isEmpty(body) && "x-www-form-urlencoded".equals(mediaType.subtype())) {
